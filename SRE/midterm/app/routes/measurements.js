  ...
  try {
    ...
    if (!["field1", "field2", "field3"].includes(field)) {
      return res.status(400).json({
        error: "Invalid field name. Must be field1, field2, or field3",
      });
    }
...
router.get("/date-range", async (req, res) => {
  try {
    const minDate = await Measurement.findOne()
      .sort({ timestamp: 1 })
      .select("timestamp -_id")
      .lean();
    const maxDate = await Measurement.findOne()
      .sort({ timestamp: -1 })
      .select("timestamp -_id")
      .lean();
    ...
    if (!minDate || !maxDate) {
      return res
        .status(404)
        .json({ error: "No data available in the database" });
    }
