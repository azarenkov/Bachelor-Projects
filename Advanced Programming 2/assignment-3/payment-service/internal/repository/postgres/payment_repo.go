...
func (r *PaymentRepository) Save(ctx context.Context, p *domain.Payment) error {
	...
	_, err := r.db.ExecContext(ctx, query,
		p.ID, p.OrderID, p.TransactionID, p.Amount, p.CustomerEmail, p.Status, p.CreatedAt,
	)
	return err
}
...
func (r *PaymentRepository) FindByOrderID(ctx context.Context, orderID string) (*domain.Payment, error) {
	query := `
		SELECT id, order_id, transaction_id, amount, customer_email, status, created_at
		FROM payments WHERE order_id = $1`
	row := r.db.QueryRowContext(ctx, query, orderID)

	var p domain.Payment
	err := row.Scan(&p.ID, &p.OrderID, &p.TransactionID, &p.Amount, &p.CustomerEmail, &p.Status, &p.CreatedAt)
