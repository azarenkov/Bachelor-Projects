...
describe("SimpleToken Contract - Comprehensive Tests", function () {
  ...
  describe("3. Failing Transfer Tests", function () {
    it("Should fail when insufficient balance", async function () {
      const { token, owner, addr1 } = await loadFixture(deployTokenFixture);
      await expect(token.connect(addr1).transfer(owner.address, ethers.parseUnits("1", 18)))
        .to.be.revertedWith("Insufficient balance");
    });

    it("Should fail when transferring more than balance", async function () {
      const { token, owner, addr1 } = await loadFixture(deployTokenFixture);
      await expect(token.transfer(addr1.address, ethers.parseUnits("1001", 18)))
        .to.be.revertedWith("Insufficient balance");
    });

    it("Should fail when transferring to zero address", async function () {
      const { token } = await loadFixture(deployTokenFixture);
      await expect(token.transfer(ethers.ZeroAddress, ethers.parseUnits("1", 18)))
  ...
  describe("5. Gas Estimation Tests", function () {
    it("Should estimate gas for transfer", async function () {
      const { token, addr1 } = await loadFixture(deployTokenFixture);
      const gasEstimate = await token.transfer.estimateGas(addr1.address, ethers.parseUnits("10", 18));
    ...
    it("Should track actual gas used for transfer", async function () {
      const { token, addr1 } = await loadFixture(deployTokenFixture);
      const tx = await token.transfer(addr1.address, ethers.parseUnits("10", 18));
    ...
    it("Should compare gas for different transfer amounts", async function () {
      ...
      const tx1 = await token.transfer(addr1.address, ethers.parseUnits("1", 18));
      const receipt1 = await tx1.wait();
      const tx2 = await token.transfer(addr2.address, ethers.parseUnits("999", 18));
    ...
    it("Should estimate gas for approve", async function () {
      const { token, addr1 } = await loadFixture(deployTokenFixture);
      const gasEstimate = await token.approve.estimateGas(addr1.address, ethers.parseUnits("100", 18));
  ...
  describe("6. Event Emission Tests", function () {
    it("Should emit Transfer event on deployment", async function () {
      ...
      const SimpleToken = await ethers.getContractFactory("SimpleToken");
      const token = await SimpleToken.deploy("Test", "TST", 18, ethers.parseUnits("100", 18));
    ...
    it("Should emit multiple Transfer events", async function () {
      ...
      await expect(token.transfer(addr1.address, ethers.parseUnits("10", 18)))
        .to.emit(token, "Transfer");
      await expect(token.transfer(addr2.address, ethers.parseUnits("20", 18)))
        .to.emit(token, "Transfer");
    });
    ...
    it("Should emit Transfer on transferFrom", async function () {
      ...
      await expect(token.connect(addr1).transferFrom(owner.address, addr2.address, ethers.parseUnits("50", 18)))
        .to.emit(token, "Transfer")
    ...
    it("Should emit Transfer on burn", async function () {
      ...
      await expect(token.burn(ethers.parseUnits("10", 18)))
        .to.emit(token, "Transfer")
        .withArgs(owner.address, ethers.ZeroAddress, ethers.parseUnits("10", 18));
    });
  });
  ...
  describe("7. Storage Verification", function () {
    ...
    it("Should update totalSupply after burn", async function () {
      ...
      expect(await token.totalSupply()).to.equal(initialSupply - ethers.parseUnits("100", 18));
    });
  });
  ...
  describe("8. Negative Tests", function () {
    ...
    describe("8.3 TransferFrom Negative Tests", function () {
      ...
      it("Should revert when transferring from zero address", async function () {
        const { token, addr1 } = await loadFixture(deployTokenFixture);
        await expect(token.transferFrom(ethers.ZeroAddress, addr1.address, 1))
      ...
      it("Should revert when transferring to zero address", async function () {
        ...
        await expect(token.connect(addr1).transferFrom(owner.address, ethers.ZeroAddress, 1))
          .to.be.revertedWith("Cannot transfer to zero address");
      });
      ...
      it("Should revert on zero amount transferFrom", async function () {
        const { token, owner, addr1, addr2 } = await loadFixture(deployTokenFixture);
        await token.approve(addr1.address, 100);
        await expect(token.connect(addr1).transferFrom(owner.address, addr2.address, 0))
          .to.be.revertedWith("Transfer amount must be greater than zero");
      });
    });
    ...
    describe("8.5 Burn Negative Tests", function () {
      it("Should revert burn with insufficient balance", async function () {
        const { token, addr1 } = await loadFixture(deployTokenFixture);
        await expect(token.connect(addr1).burn(1))
      ...
      it("Should revert burn more than balance", async function () {
        const { token, owner } = await loadFixture(deployTokenFixture);
        const balance = await token.balanceOf(owner.address);
        await expect(token.burn(balance + 1n))
          .to.be.revertedWith("Insufficient balance to burn");
      });
    });
    ...
    describe("8.6 Edge Cases with Large Numbers", function () {
      ...
      it("Should revert on transfers exceeding balance", async function () {
        const { token, owner, addr1 } = await loadFixture(deployTokenFixture);
        const balance = await token.balanceOf(owner.address);
        await expect(token.transfer(addr1.address, balance + ethers.parseUnits("1", 18)))
          .to.be.revertedWith("Insufficient balance");
      });
    });
  });
  ...
  describe("9. Advanced Integration Tests", function () {
    it("Should handle complex transfer scenario", async function () {
      ...
      expect(await token.balanceOf(owner.address)).to.equal(ethers.parseUnits("700", 18));
      expect(await token.balanceOf(addr1.address)).to.equal(ethers.parseUnits("50", 18));
      expect(await token.balanceOf(addr2.address)).to.equal(ethers.parseUnits("200", 18));
      expect(await token.balanceOf(addr3.address)).to.equal(ethers.parseUnits("50", 18));
    });
    ...
    it("Should handle mint and burn operations", async function () {
      ...
      await token.burn(ethers.parseUnits("100", 18));
      expect(await token.totalSupply()).to.equal(initialSupply + ethers.parseUnits("400", 18));
    });
  });
});
